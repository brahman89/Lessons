...
kb_cal = InlineKeyboardMarkup(resize_keyboard=True, one_time_keyboard=True).row(button_1, button_2)


@dp.callback_query_handler(text='start')
async def main_menu(call):
    await call.message.answer("Привет! Я бот помогающий твоему здоровью.\nВыберите опцию:", reply_markup=kb_cal)
    await call.answer()
