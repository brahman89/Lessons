...
kb_prod = InlineKeyboardMarkup(resize_keyboard=True).row(b_pr_1, b_pr_2, b_pr_3, b_pr_4)


@dp.callback_query_handler(text='Купить')
async def get_buying_list(call):
    for i in range(1,5):
        await call.message.answer(f'Название: Product{i}| Описание: описание {i} | Цена: {i*100}')
...
async def main_menu(call):
    await call.message.answer(f"Привет {call.message.from_user.username}! Я бот помогающий твоему здоровью.\nВыберите опцию:", reply_markup=kb_cal)
    await call.answer()
...
async def get_formulas(call):
    await call.message.answer('для женщины: 10 * вес(кг) + 6.25 * рост (см) – 4.92 * возраст – 161\nдля мужчины: 10 * вес (кг) + 6.25 * рост (см) – 4.92 * возраст + 5')
    await call.answer()
